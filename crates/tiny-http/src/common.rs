...
impl StatusCode {
    ...
    pub const fn default_reason_phrase(&self) -> &'static str {
        match self.0 {
            100 => "Continue",
...
impl From<i8> for StatusCode {
    fn from(in_code: i8) -> Self {
        Self(in_code as u16)
    }
}

impl From<u8> for StatusCode {
    fn from(in_code: u8) -> Self {
        Self(in_code as u16)
    }
}

impl From<i16> for StatusCode {
    fn from(in_code: i16) -> Self {
        Self(in_code as u16)
    }
}

impl From<u16> for StatusCode {
    fn from(in_code: u16) -> Self {
        Self(in_code)
    }
}

impl From<i32> for StatusCode {
    fn from(in_code: i32) -> Self {
        Self(in_code as u16)
    }
}

impl From<u32> for StatusCode {
    fn from(in_code: u32) -> Self {
        Self(in_code as u16)
    }
}
...
impl Header {
    ...
    pub fn from_bytes<B1, B2>(header: B1, value: B2) -> Result<Self, ()>
    where
...
impl HeaderField {
    pub fn from_bytes<B>(bytes: B) -> Result<Self, FromAsciiError<B>>
    where
...
impl Ord for HTTPVersion {
    fn cmp(&self, other: &Self) -> Ordering {
        let Self(my_major, my_minor) = *self;
        let Self(other_major, other_minor) = *other;
...
impl PartialOrd<(u8, u8)> for HTTPVersion {
    fn partial_cmp(&self, &(major, minor): &(u8, u8)) -> Option<Ordering> {
        self.partial_cmp(&Self(major, minor))
    }
}
