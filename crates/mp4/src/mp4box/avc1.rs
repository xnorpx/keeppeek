...
impl AvcCBox {
    pub fn new(sps: &[u8], pps: &[u8]) -> Self {
        let (avc_profile_indication, profile_compatibility, avc_level_indication) =
            if sps.len() >= 4 {
                (sps[1], sps[2], sps[3])
            } else {
                (0, 0, 0)
        ...
        Self {
            ...
            sequence_parameter_sets: if sps.is_empty() {
                vec![]
            } else {
                vec![NalUnit::from(sps)]
            },
            picture_parameter_sets: if pps.is_empty() {
                vec![]
            } else {
                vec![NalUnit::from(pps)]
            },
        }
    }
}
