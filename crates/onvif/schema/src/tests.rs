...
fn nested_structs_with_same_named_fields() {
    ...
    let ser = r#"<?xml version="1.0" encoding="utf-8"?>
        <tt:Profile token="test" xmlns:tt="http://www.onvif.org/ver10/schema">
            <tt:Name>TestProfile</tt:Name>
            <tt:Extension />
        </tt:Profile>
        "#;
    ...
    assert_eq!(
        des,
        tt::Profile {
            name: tt::Name("TestProfile".to_string()),
            token: tt::ReferenceToken("test".to_string()),
...
fn operation_pull_messages() {
                ...
                <tev:PullMessagesResponse
                    ...
                    <wsnt:NotificationMessage>
                        <wsnt:SubscriptionReference>
                            <wsa5:Address>http://example.com/subscription</wsa5:Address>
                        </wsnt:SubscriptionReference>
                        <wsnt:Topic
                        ...
                        <wsnt:ProducerReference>
                            <wsa5:Address>http://example.com/producer</wsa5:Address>
                        </wsnt:ProducerReference>
                        <wsnt:Message>
                            <tt:Message
    ...
    let response = event::pull_messages(&transport, &req);
...
#[test]
#[cfg(feature = "event")]
fn operation_create_pullpoint_subscription() {
    ...
    let resp = event::create_pull_point_subscription(&transport, &req).unwrap();
