...
#[yaserde(
    prefix = "tev",
    namespaces = { "tev" = "http://www.onvif.org/ver10/events/wsdl" }
)]
...
pub struct Capabilities {
    ...
    #[yaserde(attribute = true, rename = "WSSubscriptionPolicySupport")]
    pub ws_subscription_policy_support: Option<bool>,

    // Indicates that the WS Pull Point is supported.
    #[yaserde(attribute = true, rename = "WSPullPointSupport")]
...
pub fn create_pull_point_subscription<T: transport::Transport>(
    transport: &T,
...
// topics supported by
// the device.
pub fn get_event_properties<T: transport::Transport>(
    transport: &T,
...
pub fn pull_messages<T: transport::Transport>(
    transport: &T,
...
pub fn seek<T: transport::Transport>(
    transport: &T,
    request: &Seek,
...
// command.
// pub fn unsubscribe<T: transport::Transport>(
//     transport: &T
