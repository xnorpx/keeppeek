...
fn main() {
    ...
    let camera: Camera = Camera {
        device_service_url: format!("http://{camera_ip}/onvif/device_service"),
        username: username.to_string(),
        password: password.to_string(),
        event_service_url: format!("http://{camera_ip}/onvif/event_service"),
    };
    ...
    let create_pull_puint_sub_response =
        event::create_pull_point_subscription(&event_client, &create_pull_sub_request);
    ...
    // Main Loop
    loop {
        let pull_messages_response = event::pull_messages(&pull_msg_client, &pull_messages_request);
