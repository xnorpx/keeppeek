...
const REQUEST_FUNC_BODY: &str = "transport::request(transport, request)";
...
fn generate_function(func: &Function<'_>, target_ns: Option<&Namespace>) -> String {
    ...
    let mut arguments = func
        .arguments
        .iter()
        .map(|parameter| format!("{}: &{}", parameter.name, ftype(parameter.typename)))
        .collect::<Vec<String>>()
        .join(",\n    ");
...
fn split_name(name: &str) -> (Option<&str>, &str) {
    name.find(':').map_or((None, name), |index| {
...
fn sanitize(s: String) -> String {
    if s.is_empty() {
        s
    } else if s.chars().next().is_some_and(|c| c.is_numeric()) || RS_KEYWORDS.contains(&s.as_str())
    {
        format!("_{s}")
    } else {
        s
