...
impl<'a> Definitions<'a> {
    ...
    pub const fn imports(&self) -> &HashMap<&'a str, Import<'a>> {
        &self.imports
    }

    pub const fn port_types(&self) -> &HashMap<&'a str, PortType<'a>> {
        &self.port_types
    }
    ...
    pub const fn messages(&self) -> &HashMap<&'a str, Message<'a>> {
        &self.messages
    }

    pub fn get_message_by_param(&self, param: &Param<'_>) -> Option<&Message<'_>> {
        self.messages
            .get(param.message().split(':').next_back().unwrap())
    }
...
impl<'a> Import<'a> {
    ...
    pub const fn new(node: &Node<'a, '_>) -> Self {
        Self { node: *node }
    }
}
