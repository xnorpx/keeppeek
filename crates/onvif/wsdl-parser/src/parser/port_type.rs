use crate::parser::{ElementType, WsdlElement, constants::attribute};
use roxmltree::Node;
...
impl<'a> PortType<'a> {
    ...
    pub fn operations(&self) -> &[Operation<'_>] {
...
impl<'a> Operation<'a> {
    ...
    pub const fn operation_type(&self) -> &OperationType<'_> {
        &self.ty
    }
        ...
        while let Some(ch) = children.next() {
            match ch.wsdl_type() {
                ElementType::Input => {
                    ...
                    return children.next().map_or_else(
