...
use chrono::{FixedOffset, NaiveDate, format::strftime::StrftimeItems};
...
impl Date {
    pub const fn from_chrono_naive_date(date: NaiveDate) -> Self {
        Self {
            value: date,
            timezone: None,
        }
    ...
    pub const fn to_chrono_naive_date(&self) -> NaiveDate {
        self.value
    }
}
...
impl FromStr for Date {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s.matches('-').count() == 3 {
            let idx: usize = s.match_indices('-').nth(2).unwrap().0;
            let date_token = &s[..idx];
            let tz_token = &s[idx..];
            return Ok(Self {
                value: parse_naive_date(date_token)?,
