...
#[derive(Default, Clone, PartialEq, Eq, PartialOrd, Debug, UtilsDefaultSerde)]
pub struct DateTimeStamp {
    pub value: DateTime,
}

impl DateTimeStamp {
    pub const fn from_chrono_datetime(datetime: CDateTime<FixedOffset>) -> Self {
        Self {
            value: DateTime::from_chrono_datetime(datetime),
        }
    }

    pub const fn to_chrono_datetime(&self) -> CDateTime<FixedOffset> {
...
impl FromStr for DateTimeStamp {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match CDateTime::parse_from_rfc3339(s) {
            Ok(cdt) => Ok(Self::from_chrono_datetime(cdt)),
            Err(err) => Err(err),
        }
    }
}
