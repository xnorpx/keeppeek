    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        fn fill_component(
            ...
            if context.is_number_empty {
                return Err(format!(
                    "No value is specified for {name}, so '{symbol}' must not be present"
                ));
            }
        ...
        let mut dur: Self = Default::default();
...
mod tests {
    ...
    fn check_valid(s: &str) {
        if let Err(err) = Duration::from_str(s) {
            panic!("{s} should be valid, but an error occurred: {err}")
        }
    }

    fn check_invalid(s: &str) {
        if Duration::from_str(s).is_ok() {
            panic!("{s} should be invalid")
        }
    }
