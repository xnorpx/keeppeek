...
impl FromStr for GMonth {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(s) = s.strip_suffix('Z') {
            return Self::new(parse_value(s)?, Some(FixedOffset::east_opt(0).unwrap()));
        }
        ...
        if s.contains('+') {
            ...
            let tz_token = &s[idx..];
            return Self::new(parse_value(value_token)?, Some(parse_timezone(tz_token)?));
        }
