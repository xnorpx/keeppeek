...
#[derive(PartialEq, Eq, Debug, Clone, UtilsDefaultSerde)]
...
impl GMonthDay {
    ...
    pub const fn gmonth(self) -> GMonth {
        GMonth {
            value: self.month,
...
impl FromStr for GMonthDay {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Some(s) = s.strip_suffix('Z') {
            ...
            return Self::new(month, day, Some(FixedOffset::east_opt(0).unwrap()));
        }
        ...
        if s.contains('+') {
            ...
            let idx: usize = s.match_indices('+').nth(0).unwrap().0;
            let value_token = &s[..idx];
            let tz_token = &s[idx..];
            let (month, day) = parse_value(value_token)?;
            return Self::new(month, day, Some(parse_timezone(tz_token)?));
        }

        if s.matches('-').count() == 4 {
            let idx: usize = s.match_indices('-').nth(3).unwrap().0;
            let value_token = &s[..idx];
            let tz_token = &s[idx..];
            let (month, day) = parse_value(value_token)?;
            return Self::new(month, day, Some(parse_timezone(tz_token)?));
