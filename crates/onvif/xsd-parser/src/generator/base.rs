...
pub trait BaseGenerator {
    ...
    fn format_type_name(&self, type_name: &str, generator: &Generator) -> Cow<'_, str> {
        if let Some(t) = match_built_in_type(type_name, &generator.xsd_ns.borrow()) {
            return t.into();
        }
        default_format_type(type_name, &generator.target_ns.borrow())
    }
    ...
    fn join_subtypes(&self, subtypes: &[RsEntity], generator: &Generator) -> String {
        subtypes
            .iter()
            .map(|f| generator.generate(f))
            .collect::<Vec<String>>()
            .join("\n")
    }
}
