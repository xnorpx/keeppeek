...
use crate::{
    generator::{
        alias::AliasGenerator, base::BaseGenerator, r#enum::EnumGenerator,
        enum_case::EnumCaseGenerator, import::ImportGenerator, r#struct::StructGenerator,
...
mod test {
    ...
    fn test_generate_rs_file() {
        ...
        assert!(generator.generate_rs_file(&rs_file).is_empty());
        ...
        let expected = format!("{comment}{macros}pub struct Name (pub Type);\n\n{validation}");
        assert_eq!(generator.generate_rs_file(&rs_file), expected);
    }
}
