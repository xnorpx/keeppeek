...
pub fn yaserde_for_attribute(name: &str, indent: &str) -> String {
    name.find(':').map_or_else(
        || format!("{indent}#[yaserde(attribute = true, rename = \"{name}\")]\n"),
        |index| {
            format!(
...
pub fn yaserde_for_element(
    ...
    let (prefix, field_name) = name.find(':').map_or_else(
        || {
            (
                target_namespace.and_then(|namespace| namespace.name()),
                name,
            )
        },
        |index| (Some(&name[0..index]), &name[index + 1..]),
    );

    prefix.map_or_else(
        || format!("{indent}#[yaserde(rename = \"{field_name}\")]\n"),
        |prefix| format!("{indent}#[yaserde(prefix = \"{prefix}\", rename = \"{field_name}\")]\n"),
    )
}

pub fn yaserde_for_flatten_element(indent: &str) -> String {
    format!("{indent}#[yaserde(flatten = true)]\n")
}
