...
impl<'a> XsdNode for roxmltree::Node<'a, '_> {
    fn xsd_type(&self) -> ElementType {
        ...
        match self.tag_name().name() {
            ...
            "element" => Element,
            "extension" => self.parent_element().map_or_else(
                || XsdError("'extension' element must have a parent".to_string()),
                |parent| match parent.tag_name().name() {
            ...
            "notation" => Notation,
            "redefine" => Redefine,
            "restriction" => self.parent_element().map_or_else(
                || XsdError("'restriction' element must have a parent".to_string()),
                |parent| match parent.tag_name().name() {
            ...
            "whiteSpace" => self.attr_value().map_or_else(
                || unreachable!("Value is required for facets"),
                |value| match value {
                    ...
                    value => unreachable!("Invalid WhiteSpace value: {}.\n {:?}", value, self),
                },
            ),
...
pub fn max_occurs(node: &roxmltree::Node) -> MaxOccurs {
    node.attribute(attribute::MAX_OCCURS)
        .map_or(MaxOccurs::None, |value| match value {
