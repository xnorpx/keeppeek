...
use crate::uany::{UnsafeAny, UnsafeAnyExt};
...
use crate::internals::Implements;
pub use crate::internals::{CloneAny, DebugAny};
...
const fn _assert_types() {
    use std::fmt::Debug;

    const fn _assert_send<T: Send>() {}
    const fn _assert_sync<T: Sync>() {}
    const fn _assert_clone<T: Clone>() {}
    const fn _assert_debug<T: Debug>() {}
...
impl<A: UnsafeAnyExt + ?Sized> TypeMap<A> {
    ...
    /// # Safety
    ///
    /// The map must preserve the invariant that each `TypeId` identifies the concrete value type
    /// stored for that key. Violating the invariant can make later `TypeMap` lookups perform an
    /// invalid unchecked downcast.
    pub const unsafe fn data(&self) -> &HashMap<TypeId, Box<A>> {
        &self.data
    }
    ...
    /// # Safety
    ///
    /// Every mutation must preserve the invariant that each `TypeId` identifies the concrete
    /// value type stored for that key. Violating the invariant can make later `TypeMap` lookups
    /// perform an invalid unchecked downcast.
    pub const unsafe fn data_mut(&mut self) -> &mut HashMap<TypeId, Box<A>> {
        &mut self.data
    }
