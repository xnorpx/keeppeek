...
impl fmt::Display for PostError {
    ...
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        let description = match *self {
            ...
            Self::WrongContentType => "the request didn't have a post content type",
            Self::IoError(_) => {
            ...
            Self::Field { .. } => "failed to parse a requested field",
        };
...
impl error::Error for PostFieldError {
    ...
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match *self {
            Self::IoError(ref e) => Some(e),
            Self::WrongDataTypeInt(ref e) => Some(e),
            Self::WrongDataTypeFloat(ref e) => Some(e),
            _ => None,
        }
    }
}
...
impl fmt::Display for PostFieldError {
    ...
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        let description = match *self {
            ...
            Self::MissingField => "the field is missing from the request's client",
            Self::WrongFieldType => "expected a file but got a field, or vice versa",
            Self::UnexpectedMultipleValues => {
                "got multiple values for the same field while only one was expected"
            }
            Self::WrongDataTypeInt(_) => "failed to parse an integer field",
            Self::WrongDataTypeFloat(_) => "failed to parse a floating-point field",
        };
    ...
    T: DecodePostField<C>,
{
    fn from_field(config: C, content: &str) -> Result<Self, PostFieldError> {
        DecodePostField::from_field(config, content).map_or_else(|_| Ok(None), |val| Ok(Some(val)))
    }
    ...
    where
        R: BufRead,
    {
        DecodePostField::from_file(config, file, filename, mime)
...
impl DecodePostField<()> for bool {
    ...
    #[inline]
    fn merge_multiple(self, existing: Self) -> Result<Self, PostFieldError> {
        Ok(self || existing)
    }
    ...
    T: DecodePostField<C>,
{
    ...
    fn merge_multiple(mut self, mut existing: Self) -> Result<Self, PostFieldError> {
