    ...
    F: Send + Sync + 'static + Fn(&Request) -> Response,
{
    ...
    pub fn new<A>(addr: A, handler: F) -> Result<Self, Box<dyn Error + Send + Sync + 'static>>
    where
        A: ToSocketAddrs,
    {
    ...
    /// Builds a server from an existing TCP listener.
    ...
    /// Waits up to `timeout` for all in-flight requests to finish.
    ///
    /// Returns `false` when one or more request workers remain active after the timeout.
    pub fn join_timeout(&self, timeout: Duration) -> bool {
        self.executor.join_timeout(timeout)
    }
...
impl Request {
    ...
    pub fn fake_http<U, M>(method: M, url: U, headers: Vec<(String, String)>, data: Vec<u8>) -> Self
    where
        U: Into<String>,
        M: Into<String>,
    {
    ...
    pub fn raw_query_string(&self) -> &str {
        self.url
            .bytes()
            .position(|c| c == b'?')
            .map_or("", |pos| self.url.split_at(pos + 1).1)
    }
...
mod tests {
    ...
    fn join_timeout_returns_while_a_client_is_not_reading() {
        ...
        let server = Server::from_tcp_listener(listener, move |_| {
            let _ = started_tx.send(());
            Response {
                status_code: 200,
                headers: Vec::new(),
                data: ResponseBody::from_reader_and_size(
                    std::io::repeat(0).take(RESPONSE_BYTES as u64),
                    RESPONSE_BYTES,
                ),
                upgrade: None,
            }
        ...
        assert!(!server.join_timeout(Duration::from_millis(50)));
