...
impl fmt::Display for WebsocketError {
    ...
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        let description = match *self {
            Self::InvalidWebsocketRequest => "the request does not match a websocket request",
    ...
    S: Into<Cow<'static, str>>,
{
    ...
    if let Some(ref sp) = subprotocol
        && !requested_protocols(request).any(|p| &p == sp)
    {
...
pub fn requested_protocols(request: &Request) -> RequestedProtocolsIter {
    request.header("Sec-WebSocket-Protocol").map_or_else(
