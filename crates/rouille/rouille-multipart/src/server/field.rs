...
use super::httparse::{EMPTY_HEADER, Error as HttparseError, Header, Status};
...
macro_rules! invalid_cont_disp {
    ($reason: expr_2021, $cause: expr_2021) => {
...
impl FieldHeaders {
    ...
    fn parse(headers: &[StrHeader]) -> Result<Self, ParseHeaderError> {
...
impl ContentDisp {
    fn parse_required(headers: &[StrHeader]) -> Result<Self, ParseHeaderError> {
...
impl<M: ReadEntry> MultipartField<M> {
    ...
    pub fn is_text(&self) -> bool {
        self.headers
            .content_type
            .as_ref()
            .is_none_or(|ct| ct.type_() == mime::TEXT)
    }
    ...
    M: ReadEntry,
{
    ...
    const fn inner_mut(&mut self) -> &mut M {
        self.inner.as_mut().expect(DATA_INNER_ERR)
    }

    const fn take_inner(&mut self) -> M {
...
pub trait ReadEntry: PrivReadEntry + Sized {
    ...
    fn read_entry(mut self) -> ReadEntryResult<Self> {
            ...
            && ct.type_() == mime::MULTIPART
        {
            ...
            info!(
                "Found nested multipart field: {field_headers:?}:\r\n\
...
impl<M: ReadEntry> PrivReadEntry for &mut M {
...
impl<M: ReadEntry, Entry> ReadEntryResult<M, Entry> {
    ...
    pub fn into_result(self) -> io::Result<Option<Entry>> {
        match self {
            Self::Entry(entry) => Ok(Some(entry)),
            Self::End(_) => Ok(None),
            Self::Error(_, err) => Err(err),
        }
    }
...
quick_error! {
    ...
    enum ParseHeaderError {
        ...
        MissingContentDisposition(headers: String) {
            display("\"Content-Disposition\" header not found in field headers:\n{}", headers)
        }
        InvalidContDisp(reason: &'static str, cause: String) {
            display("invalid \"Content-Disposition\" header: {}: {}", reason, cause)
        }
        /// The header was found but could not be parsed
        TokenizeError(err: HttparseError) {
            display("an error occurred while parsing field headers: {}", err)
            source(err)
            from()
        }
        MimeError(cont_type: String) {
            display("Failed to parse Content-Type: {}", cont_type)
        }
        TooLarge {
        ...
        /// IO error
        Io(err: io::Error) {
            display("an io error occurred while parsing the headers: {}", err)
            source(err)
            from()
        }
    }
