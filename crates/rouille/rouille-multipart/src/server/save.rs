    ...
    pub fn with_entries(self, mut entries: Entries) -> EntriesSaveResult<M> {
        ...
        let save_field = |field: &mut MultipartField<M>, entries: &Entries| {
            ...
            let mut saver = SaveBuilder {
                savable: &mut field.data,
                open_opts: open_opts.clone(),
                count_limit,
                size_limit,
                memory_threshold,
                text_policy,
    ...
    MultipartData<M>: BufRead,
{
    ...
    pub fn with_path<P: Into<PathBuf>>(&mut self, path: P) -> FieldSaveResult {
        ...
        let data = try_full!(
            try_write_all(&bytes, &mut file).map(move |size| SavedData::File(path, size as u64))
        );
    ...
    pub fn write_to<W: Write>(&mut self, mut dest: W) -> SaveResult<u64, u64> {
        if self.size_limit < u64::MAX {
            try_copy_limited(
                &mut self.savable,
                |buf| try_write_all(buf, &mut dest),
                self.size_limit,
            )
        } else {
    ...
    fn read_mem<Wb: FnMut(&[u8]) -> SaveResult<usize, usize>>(
        &mut self,
        with_buf: Wb,
        pre_read: u64,
    ) -> SaveResult<u64, u64> {
        let limit = cmp::min(self.size_limit, self.memory_threshold).saturating_sub(pre_read);
        try_copy_limited(&mut self.savable, with_buf, limit)
    }

    const fn cmp_size_limit(&self, size: usize) -> bool {
...
impl Entries {
    ...
    pub const fn fields_count(&self) -> u32 {
        self.fields_count
    }
    ...
    fn push_field(&mut self, mut headers: FieldHeaders, data: SavedData) {
        ...
        match self.fields.entry(headers.name.clone()) {
            Vacant(vacant) => {
                vacant.insert(vec![SavedField { headers, data }]);
            }
            Occupied(occupied) => {
        ...
        for (name, entries) in &self.fields {
            ...
            for (idx, field) in entries.iter().enumerate() {
                ...
                writeln!(
                    writer,
                    "{}: {:?} ({:?}):",
                    idx, headers.filename, headers.content_type
                )?;
...
fn create_dir_all(path: &Path) -> io::Result<()> {
    path.parent().map_or_else(
        || {
            ...
            warn!(
                "Attempting to save file in what looks like a root directory. File path: {path:?}"
            );
            Ok(())
        },
        fs::create_dir_all,
...
fn try_copy_limited<R: BufRead, Wb: FnMut(&[u8]) -> SaveResult<usize, usize>>(
    src: R,
    mut with_buf: Wb,
    limit: u64,
) -> SaveResult<u64, u64> {
    ...
    try_read_buf(src, |buf| {
        ...
        if new_copied > limit {
            return Partial(0, PartialReason::SizeLimit);
        }
        copied = new_copied;
...
fn try_read_buf<R: BufRead, Wb: FnMut(&[u8]) -> SaveResult<usize, usize>>(
    mut src: R,
    mut with_buf: Wb,
) -> SaveResult<u64, u64> {
