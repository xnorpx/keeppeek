...
impl<'a> From<LazyError<'a, Self>> for io::Error {
    fn from(val: LazyError<'a, Self>) -> Self {
        val.error
    }
}
...
impl<'d> PreparedFields<'d> {
    ...
    pub const fn content_len(&self) -> Option<u64> {
        self.content_len
    }
    ...
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        while total_read < buf.len() && !cursor_at_end(&self.end_boundary) {
            ...
            total_read += if !cursor_at_end(&self.text_data) {
                ...
                match stream {
                    Some(mut field) => match field.read(buf) {
                        Ok(0) => continue,
                        result => {
                    ...
                    None => self.end_boundary.read(buf)?,
                }
            };
        }
