...
impl FileEntry {
    fn from_field<M: ReadEntry>(mut field: MultipartField<M>) -> (Self, M) {
...
macro_rules! do_test (
    ($client_test:ident, $server_test:ident) => (
        ...
        let mut test_fields = TestFields::r#gen();
...
mod extended {
    use super::{TestFields, test_client, test_client_lazy, test_server, test_server_entry_api};
...
fn gen_string() -> String {
    ...
    rng_1
        ...
        .take(str_len_1)
        .chain(iter::repeat_n(b'-', num_dashes))
