...
impl<'i> Contiguous<'i> {
    pub const fn new(data: &'i [u8], partial: bool) -> Self {
        Self { data, partial }
    }
}
...
impl<'i> Split<'i> {
    pub const fn new(first: &'i [u8], second: &'i [u8], partial: bool) -> Self {
        if first.is_empty() {
            Self {
                first: second,
                second: &[],
...
impl<'i> Input<'i> for Split<'i> {
    ...
    fn byte_at(&self, i: usize) -> u8 {
        i.checked_sub(self.first.len())
            .map_or_else(|| self.first[i], |beyond_first| self.second[beyond_first])
    }
