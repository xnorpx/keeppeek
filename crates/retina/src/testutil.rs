...
/// Deterministic no-I/O driver for the SANS-I/O RTSP core.
#[cfg(test)]
pub(crate) struct ScriptedDriver {
    client: crate::client::core::RtspClient,
...
#[cfg(test)]
impl ScriptedDriver {
    pub(crate) fn new(client: crate::client::core::RtspClient) -> Self {
        Self {
            client,
    ...
    pub(crate) fn time(&self, offset: Duration) -> crate::client::core::Time {
        crate::client::core::Time {
            monotonic: self.monotonic_base + offset,
            wall: self.wall_base + offset,
        }
    }

    pub(crate) fn handle(
        &mut self,
        input: crate::client::core::Input<'_>,
    ) -> Result<Vec<crate::client::core::Output>, crate::client::core::CoreError> {
        self.client.handle_input(input)?;
        Ok(self.drain())
    }

    pub(crate) fn client(&self) -> &crate::client::core::RtspClient {
        &self.client
    }

    pub(crate) fn drain(&mut self) -> Vec<crate::client::core::Output> {
        let mut outputs = Vec::new();
        loop {
            let output = self.client.poll_output();
            let done = matches!(output, crate::client::core::Output::Timeout(_));
            outputs.push(output);
            if done {
                return outputs;
            }
