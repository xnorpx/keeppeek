...
impl Depacketizer {
    pub(super) fn new(
        ...
        let parameters = format_specific_params.map_or_else(
            || None,
            |fp| match InternalParameters::parse_format_specific_params(fp) {
                Ok(p) => Some(p),
                Err(e) => {
    ...
    fn push_inner(&mut self, pkt: ReceivedPacket) -> Result<(), String> {
                    ...
                    if loss > 0 {
                        ...
                        if access_unit.in_fu {
                            ...
                            let last_nal_hdr = self
                                .nals
                                .last()
                                .ok_or_else(|| "nals should not be empty".to_string())?
                                .hdr;
                            if can_end_au(last_nal_hdr.unit_type()) {
                                ...
                                log::debug!(
                                    "Bogus mid-access unit timestamp change after {last_nal_hdr:?}"
                                );
                ...
                let pieces = match (start, access_unit.in_fu) {
                    ...
                    (false, true) => {
                        ...
                        let nal = self
                            .nals
                            .last_mut()
                            .ok_or_else(|| "nals non-empty while in fu".to_string())?;
                        // TODO
        ...
        self.input_state = if mark {
            let last_nal_hdr = self
                .nals
                .last()
                .ok_or_else(|| "nals should not be empty after mark".to_string())?
                .hdr;
            if can_end_au(last_nal_hdr.unit_type()) {
                ...
                log::debug!("Bogus mid-access unit timestamp change after {last_nal_hdr:?}");
...
const fn can_end_au(nal_unit_type: nal::UnitType) -> bool {
