...
const protobufRuntimeVersion = uiManifest.dependencies?.['@bufbuild/protobuf'];
const protobufGeneratorVersion = uiManifest.devDependencies?.['@bufbuild/protoc-gen-es'];
if (!protobufGeneratorVersion || protobufRuntimeVersion !== protobufGeneratorVersion) {
	throw new Error('@bufbuild/protobuf and @bufbuild/protoc-gen-es must use the same exact version');
}
const generatedProtobuf = readFileSync(resolve(uiRoot, 'src/lib/proto/webrtc_pb.ts'), 'utf8');
const generatedVersion = /^\/\/ @generated by protoc-gen-es v(\d+\.\d+\.\d+)\b/m.exec(
	generatedProtobuf
)?.[1];
if (generatedVersion !== protobufGeneratorVersion) {
	throw new Error(
		`Generated protobuf uses protoc-gen-es ${generatedVersion ?? 'unknown'}, expected ${protobufGeneratorVersion}`
	);
}
