<script lang="ts">
	...
	let rankedFindings = $derived(health ? rankHealthFindings(health) : []);
	let primaryFinding = $derived(rankedFindings[0] ?? null);
	$effect(() => {
...
<div class="mx-auto max-w-[120rem] space-y-6">
	...
	{:else if loading}
		<div
			class="grid min-h-[calc(100svh-78px)] place-items-center text-sm text-text-muted md:hidden"
		>
			Loading server health
		</div>
	{:else if error}
		<div
			class="border-y border-live/40 bg-live/5 px-4 py-3 text-sm text-live-text md:hidden"
			role="alert"
		>
			{error}
		</div>
	...
	<div class="hidden space-y-6 md:block">
		...
		{:else if health}
			...
			{#if primaryFinding}
				<HealthPriorityCard finding={primaryFinding} status={health.status} />
			{/if}
			...
			{#if activeTab === 'client'}
				...
				>
					<section aria-labelledby="client-connection-heading">
						...
						<div class="grid grid-cols-2 divide-x divide-y border-y xl:grid-cols-7 sm:grid-cols-4">
							{#each [['Session', livePeer.sessionId === null ? '—' : `#${livePeer.sessionId}`], ['Connection', livePeer.connectionState], ['ICE', livePeer.iceConnectionState], ['Tracks', clientTracks.length], ['Main', clientMainTracks], ['Sub', clientSubTracks], ['BWE avg', formatBitrate(livePeer.estimatedBitrateBps)]] as metric (metric[0])}
								<div class="min-w-0 p-3" data-health-metric={metric[0]}>
									...
									<p class="mt-1 truncate font-mono text-sm font-semibold capitalize">
										{metric[1]}
									</p>
								</div>
							{/each}
								...
								<table class="w-full min-w-[96rem] text-left text-[11px]">
									...
									<tbody class="divide-y">
										{#each clientTracks as track (track.trackId)}
											{@const queue = clientQueues.find(
												(entry) => entry.track_id === track.trackId
											)}
											...
											<tr>
												...
												><td class="px-3 py-2 font-mono"
													...
													<p class="text-[9px] text-muted-foreground">
														{queue
															? `${compactFormatter.format(queue.written_frames)} written`
															: '—'}
													</p></td
												>
											</tr>
			...
			{:else}
				...
				>
							...
							<div class="divide-y border-y">
								{#each rankedFindings as finding, index (`${finding.issue.scope}-${finding.issue.message}-${index}`)}
									...
									>
										...
										<div class="min-w-0">
											<p class="font-semibold capitalize">{finding.issue.scope}</p>
											<p class="mt-0.5 opacity-85">{finding.issue.message}</p>
										</div>
										<div class="ml-auto flex shrink-0 items-center gap-3">
											<span class="font-mono text-[9px] uppercase opacity-60"
												>{finding.issue.severity}</span
											>
											{#if finding.camera}
												<a
													href={diagnosisHref(finding.camera)}
											...
											{#each cameraVideoStreams as stream, streamIndex (`${camera.id}-${stream.type}`)}
												<tr class="hover:bg-muted/25">
													<td class="px-3 py-2.5 align-top">
														{#if streamIndex === 0}
															<div class="flex items-center gap-2">
																<span class="size-1.5 rounded-full {stateColor(camera.state)}"
																></span>
																<span class="font-medium">{camera.name}</span>
																<span class="text-[9px] text-muted-foreground capitalize">
																	{camera.state}
						...
						<div class="grid divide-y border-y lg:grid-cols-2 lg:divide-x lg:divide-y-0">
							<div class="p-3">
								...
								<dl class="grid grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-2 text-xs">
									...
									<dd class="font-mono">
										{formatBytes(health.system.process.virtual_memory_bytes)}
									</dd>
									...
									<dd class="font-mono">
										{formatBytes(health.system.process.total_written_bytes)}
									</dd>
								</dl>
							</div>
							<div class="p-3">
					...
					<section class="border-t pt-4" aria-labelledby="webrtc-heading">
						...
						<div class="space-y-4">
							<div>
								...
								<div
									class="grid grid-cols-2 divide-x divide-y border-y xl:grid-cols-6 sm:grid-cols-3"
								>
					...
					<section class="border-t pt-4" aria-labelledby="storage-heading">
						...
						{#if health.storage.demand.streams.length > 0}<div
								class="mt-4 overflow-x-auto border-y"
							>
								...
								<table class="w-full min-w-[36rem] text-left text-[11px]">
									...
									><tbody class="divide-y"
										>{#each health.system.temperatures as temperature (temperature.label)}<tr
												><td class="px-3 py-2">{temperature.label}</td><td
													class="px-3 py-2 font-mono"
					...
					<section class="border-t pt-4" aria-labelledby="runtime-heading">
						...
						>
							...
							<div class="p-3">
								...
								<p class="mt-1 font-medium">
									{formatDuration(health.system.system_uptime_seconds)}
								</p>
								<p class="font-mono text-[10px] text-muted-foreground">
