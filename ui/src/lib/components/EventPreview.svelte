...
<FocusedMediaViewport
	mediaKey={`${event.source_id}:${event.id}:${event.canonical_attachment_id ?? ''}:${event.thumbnail_url ?? ''}`}
	aspectRatio={imageHeight > 0 ? imageWidth / imageHeight : 16 / 9}
	enabled={digitalZoom && Boolean(event.thumbnail_url)}
