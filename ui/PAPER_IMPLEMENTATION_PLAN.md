...
- **Current focus:** Establish canonical Linux Loki references from the 14 Paper-accepted frames and run the isolated Storybook/Loki workflow once the public npm registry transfer succeeds.
- **Last completed:** A real-server Playwright story now waits for decoded H.264, removes the configured test camera through WebRTC control, re-adds the same camera through production Settings, and emits a 22.000s single-stream H.264 MP4 with no retained WebM.
- **Current blockers:** The isolated Storybook 8/Loki 0.35 install still fails with public-registry `ConnectionClosed` errors before package manifests resolve, so the Storybook build and reviewed Linux references cannot run locally. `design/paper/keeppeek-nvr-v34/COVERAGE.md` is the source of truth for all deliberate product/API blockers and the 14-item Loki baseline queue.
...
- [x] **1.1** Install Paper tokens and bundled Archivo/IBM Plex Mono typography. All 80 runtime tokens are generated and checked; exact Fontsource 5.3.0 Latin weights 300–700 are integrity-locked, emitted as local production assets, and awaited before visual capture.
...
- [~] **3.3** Complete frame-by-frame Paper overlay and canonical screenshot acceptance. All 50 authored references are classified: 14 accepted and queued for Linux baselines, and 36 deliberately capability-gated with named blockers. Boards 01–03, 05, 28, and 32 are accounted contracts with no authored application raster; reviewed Linux Loki approval remains.
- [~] **3.4** Pass focused tests, real-media E2E, accessibility checks, performance budgets, Linux visual CI, and `./check.sh`. The canonical repository gate passes, including 85 Playwright scenarios; Linux Storybook/Loki remains blocked on dependency transfer and approved references.
...
| 2026-08-20 | 3.3 Board 02 tokens       | Planned → accounted contract  | 80-token hash/CSS parity; exact type counts and semantic roles; 2 Fontsource 5.3.0 families × 5 local weights with lock integrity | Explanatory token board is not used as an application raster                                                       |
