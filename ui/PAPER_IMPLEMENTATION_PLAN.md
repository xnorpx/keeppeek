...
- **Current focus:** Pass the frozen Linux Storybook/Loki workflow with the 14 reviewed baselines and close the remaining PR checks.
- **Last completed:** The public-registry Linux build produced all 50 deterministic candidates; the 14 Paper-accepted scenarios are now hash-locked as Loki references with approval metadata and a committed harness lockfile.
...
- [~] **3.4** Pass focused tests, real-media E2E, accessibility checks, performance budgets, Linux visual CI, and `./check.sh`. The canonical repository gate passes, including 85 Playwright scenarios; the frozen Linux Loki comparison remains.
