...
| 2A    | [~] In progress | Peek, layouts, fleet, focused history     | Boards 06/08/11/22/31 accepted               |
...
- [~] **2A.1** Implement and accept Peek states, layout editing, fleet virtualization, mobile Peek, and focused History-to-Keep. The full Svelte and Playwright implementation is complete; Boards 06 and 31 are Paper-overlay accepted, Boards 08 and 11 have measured blocked candidates, and remaining Paper/Loki approval is open.
...
9. **Peek, layouts, fleet, and focused history (Boards 06, 08, 11, 22, 31).** Rebuild `/` around the Paper live wall while preserving the shared `LivePeer.configure/attach` lifecycle and native video playback. Implement healthy, degraded, reconnecting, and offline tile states, quiet recording versus red failure signals, quality and focus behavior, the in-place 12-column layout editor with undo, discard, and done, Activity Focus and pinned overrides, and the visible History action that opens Keep for the focused camera without renegotiating or disturbing other live tiles. Rework `/cameras` into the fixed 56px fleet rows and a bounded fixed-row virtualizer for 127 sources. Persisted layout commands remain capability-gated when absent. Validate desktop, mobile, empty, degraded, keyboard, and light or dark frames before moving on.
...
- `ui/src/routes/+page.svelte` and `ui/src/routes/cameras/+page.svelte` — Peek wall, layout editor and focused History entry, and fleet virtualization.
...
| 06, 08, 11, 22, 31      | Peek, layouts, fleet, mobile, focused history       | [x]            | [~]          | Boards 06/31 accepted; Board 08/11 candidates; other references remain          |
...
| Peek `/`                          | Extend `ui/e2e/dashboard.e2e.ts` and `ui/e2e/peek-live.e2e.ts`; add focused layout and History-to-Keep specs | Empty, loading, live, degraded, reconnecting, offline, quality, focus, layout edit, Activity Focus, pinned override, History-to-Keep, mobile                                                     | [~]    |
...
```ts
export const HistoryToKeep = {
	parameters: {
		docs: {
			description: {
				story: 'Use History from a focused live camera to open its full timeline in Keep.'
			}
		},
		paper: {
			boardId: '5GD-0',
		...
		demo: {
			...
			purpose: 'Show the focused History link handing one camera to Keep for timeline review.',
			narration: {
				voice: 'coral',
				instructions: 'Speak clearly in a calm product-demo tone.',
				cues: [
					{ atMs: 0, text: 'First, choose one live camera.', pauseAfterMs: 250 },
					{ atMs: 2500, text: 'Then use History to open Keep for that camera.' }
				]
			},
			durationMs: 9000,
			...
			captions: [
				{ atMs: 0, text: 'Focus keeps the wall live.' },
				{ atMs: 2500, text: 'History opens Keep for this camera.' },
				{ atMs: 6500, text: 'Keep owns timeline navigation and review.' }
			]
		}
	}
};
...
| 2026-08-19 | 2A.1 Peek live wall       | Not started → in progress     | 5 view-model tests; 7 focused route, shell, and real-media Playwright scenarios; desktop and 390×844 trace review                                       | Paper image/Loki approval, layout editor, fleet, and focused history remain                                        |
| 2026-08-19 | 2A.1 layout editor        | In progress → checkpoint      | 5 layout-model tests; named Board 08 Playwright scenario; 1440×840 trace review; `./check.sh`                                                           | Persistence remains gated; fleet, focused history, and Paper/Loki approval remain                                  |
| 2026-08-19 | 2A.1 127-source fleet     | In progress → checkpoint      | 7 fleet/virtualizer tests; 2 named Playwright scenarios; desktop/mobile trace review; 23-test `./check.sh`                                              | Fleet commands remain gated; focused history and Paper/Loki approval remain                                        |
| 2026-08-19 | 2A.1 Focus-to-Keep        | In progress → implemented     | 5 timing tests; named real-media Playwright scenario; History/Keep trace review; 24-test `./check.sh`                                                   | Svelte/behavior complete; Paper image and Loki approval remain blocked                                             |
